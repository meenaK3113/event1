...
const genres = ['Music', 'Comedy', 'Product Launch', 'Tech', 'Coding', 'Health', 'Food', 'Art'];

const events = [
  { id: 1, title: 'Tech Conference', description: 'An exciting tech event!', genre: 'Tech', image: '', location: 'Tech City Convention Center', date: '2025-05-15' },
  { id: 2, title: 'Music Festival', description: 'A lively music event!', genre: 'Music', image: 'https://via.placeholder.com/300x200.png?text=Music+Festival', location: 'Music Park, City Center', date: '2025-06-01' },
  { id: 3, title: 'Comedy Night', description: 'A hilarious comedy event!', genre: 'Comedy', image: 'https://via.placeholder.com/300x200.png?text=Comedy+Night', location: 'Comedy Club, Downtown', date: '2025-07-20' }
];
...
const LandingPage = () => {
  ...
  const filteredEvents = events.filter(event => selectedGenres.length === 0 || selectedGenres.includes(event.genre));
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      <div className="w-1/5 bg-gray-800 text-white p-6">
        ...
        <ul>
          {genres.map((genre) => (
            <li key={genre} className="mb-4">
              <label className="flex items-center">
                <input type="checkbox" value={genre} checked={selectedGenres.includes(genre)} onChange={handleGenreChange} className="mr-2" />
                {genre}
              </label>
            </li>
          ))}
      ...
      <div className="w-4/5 p-6">
        ...
        <div className="grid grid-cols-3 gap-6">
          {filteredEvents.map((event) => (
            <div key={event.id} className="bg-white shadow-lg rounded-lg p-6 transition-transform transform hover:scale-105 hover:shadow-2xl">
